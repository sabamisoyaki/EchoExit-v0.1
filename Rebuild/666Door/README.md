...
タイトルの「部屋を編集する」から、部屋の中を一人称で歩きながら配置を編集できます。置いた異変はその場で動くので、儀式を試しながら部屋を作れます。追跡型に捕まった場合は、その異変が元の位置に戻るだけで編集は続きます。

| 操作 | キーボード／マウス |
| --- | --- |
| メニュー（置くもの・ステージの読込／新規／保存・タイトルへ） | Tab または Esc |
| 置く | Shift を押しながら床を見て左クリック（Shift＋R で向きを変える） |
| 回転／削除 | 置いたものを見て R ／ Delete |
| 叩く（儀式） | 左クリック |
| 保存 | F5 |

メニューを開いている間は異変が止まります。テキスト入力中はショートカットを抑止します。編集画面の操作は現在キーボードとマウスのみです。保存時に異変数、追跡型の重複、初期位置、扉への重なりなどを検証します。未対応IDは警告を表示して元データを保持します。

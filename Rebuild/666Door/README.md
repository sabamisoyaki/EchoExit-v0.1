...
| シーン | 役割 |
| --- | --- |
| `Title.unity` | タイトル画面。起動シーン。プレイヤーの位置と向きがタイトル背景の構図になる |
| `Game.unity` | 探索とラウンド進行。やり直しはシーン内で行う |
| `EditMode.unity` | 部屋の編集 |

3つの画面シーンには、カメラと当たり判定を持つ `Assets/Prefabs/Player.prefab` を置いています。FOV などのカメラ設定はこのプレハブ（またはシーン上のインスタンス）で調整します。
...
| 置く | Shift を押しながら床を見て左クリック（位置は1cm単位。Ctrl を押すと0.25m刻み） |
| 置く向き | Shift を押しながらホイールで5°ずつ（Ctrl で45°ずつ）、R で45° |
| 回転／削除 | 置いたものを見て ホイール・R ／ Delete |
| 叩く（儀式） | 左クリック |
| 保存 | F5 |

壁・家具・ほかの配置物とは重ねて置けます。照準の下に重なり率が出ます。
- 当たり判定の75%を超えて埋まるものは置けません。
- 異変は6個まで置けます。そのうち**重なりが25%を超える異変は、重なりの小さい異変を3個置いたうえで、それを超える分だけ**置けます。
- 扉と重なる場所、床の外、自分と重なる場所には置けません。
- 数値は `GameSettings.asset` の「配置の重なり」と `maximumAnomalies` で調整できます。

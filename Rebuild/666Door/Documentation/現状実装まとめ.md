...
| 置く | Shift を押している間、視線の先の床にプレビューを表示（0.25m 単位で吸着。緑なら置ける、赤なら置けない）→ 左クリック |
| 置く向きを変える | Shift＋R（45°ずつ） |
| 回転／削除 | 置いたものを見る（橙色の枠）→ R ／ Delete |
...
- **置けない場所**：床の範囲外、壁や扉と重なる場所、自分と重なる場所。保存時には再構築仕様書 §6 の検証（異変の上限、追跡型は1体まで、初期位置の保護、扉との重なり など）を行います。
